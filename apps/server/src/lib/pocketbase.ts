 ...
 * All DB writes go through the superuser (admin) API.
 */
...
export async function pbList(collection: string, filter?: string, sort?: string) {
  ...
  const res = await fetch(`${env.POCKETBASE_URL}/api/collections/${collection}/records${qs ? '?' + qs : ''}`, {
...
export async function pbView(collection: string, id: string) {
  const res = await fetch(`${env.POCKETBASE_URL}/api/collections/${collection}/records/${id}`, {
    headers: await adminHeaders(),
  })
...
export async function pbCreate(collection: string, data: Record<string, unknown>) {
  const res = await fetch(`${env.POCKETBASE_URL}/api/collections/${collection}/records`, {
    method: 'POST',
    headers: await adminHeaders(),
...
export async function pbUpdate(collection: string, id: string, data: Record<string, unknown>) {
  const res = await fetch(`${env.POCKETBASE_URL}/api/collections/${collection}/records/${id}`, {
    method: 'PATCH',
    headers: await adminHeaders(),
    body: JSON.stringify(data),
...
// ── User token verification (cached) ────────────────────────
...
export async function verifyUserToken(token: string): Promise<AuthUser | null> {
  ...
  const res = await fetch(`${env.POCKETBASE_URL}/api/collections/users/auth-refresh`, {
    method: 'POST',
  ...
  const user: AuthUser = {
    ...
    credits: r.credits ?? 0,
    credits_reset_at: r.credits_reset_at ?? '',
...
// Call whenever the server mutates a user record (plan/credits change)
